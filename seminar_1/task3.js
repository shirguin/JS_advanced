...
function filterDivs() {
  ...
  const arrayDivs = Array.from(divElements);
  const result = arrayDivs.filter((element) => element.dataset.active);
  //const result = arrayDivs.filter((element) => element.hasAttribute('data-active'));

  return result;
}

const activeDivs = filterDivs();
